...
export const loginPlayer = async ({input}: any, req: Request): Promise<string> => {
    const {email , password}: { email: string, password: string } = input;

    //const token: string = <string>req.headers['authorization'];
    //const username: string = verifyAdmin(token.substring(7));
    return (await LoginPlayerControllerProvider.getController().loginPlayer(email, password)).id;
}